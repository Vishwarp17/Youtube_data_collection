...
db = conn1["youtubes_data"]  # create database
coll = db["channels_data"]  # create collection
...
def migrate_to_sql(loaded_mongodb):
    ...
    for i in range(len(loaded_mongodb[0]) - 1):
        df_video_details.loc[i] = [loaded_mongodb[0][f"video_id_{i + 1}"]["Video_Id"],
                                   loaded_mongodb[0]["Channel_Name"]['playlist_id'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Video_Name'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Video_Description'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['PublishedAt'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['View_Count'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Like_Count'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Favorite_Count'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Comment_Count'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Duration'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Thumbnail'],
                                   loaded_mongodb[0][f"video_id_{i + 1}"]['Caption_Status']]
    ...
    for i in range(len(loaded_mongodb[0]) - 1):
        ...
        video_id = loaded_mongodb[0][f"video_id_{i + 1}"]["Video_Id"]
        comment_details = loaded_mongodb[0][f"video_id_{i + 1}"]['Comments']
...
col2.text("")
col2.text("")
search_button = col2.button(":blue[Search Youtube]")  # this button created for passing channel_id as input
...
col5.text("")
col5.text("")
Migrate_to_Sql = col5.button(":blue[Migrate from MongoDB to Sql]")
mongodb = col6.button(":blue[Load into MongoDB]")
...
if mongodb:
    try:
        try:
            ...
            loaded_mongodb = coll.insert_many([youtube_data])
            col5.warning("Channel details loaded in mongodb")
    except KeyError:
    ...
    if Migrate_to_Sql:
        try:
            try:
                ...
                col5.warning("Migrated from MongoDB to SQL Successfully")
        except IndexError:
